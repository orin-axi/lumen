...
>
> **Resolved (2026-08-23, was an unenforced gap since 2026-08-22):** `lumen-session`'s own `[dev-dependencies]` include `lumen-fixtures` (for adapter test fixtures), which itself depends on `lumen-store` (both FSL-1.1-MIT) — so `cargo test -p lumen-session` pulls FSL-licensed code into a crate documented as "standalone." This is a deliberate, explicit exception, not a violation: dev-dependencies never ship in the published crate a downstream consumer resolves via `cargo add lumen-session`, so lumen-session's *published* dependency graph stays MIT/Apache-2.0 -- only its local test/CI build sees FSL code. `deny.toml`'s `[bans]` section now enforces this precisely: `lumen-fixtures` is banned from the workspace graph except for its three legitimate dev-dependents (`lumen-cli`, `lumen-analysis`, `lumen-session`), so this is the *only* Layer-1.5-to-FSL edge that's allowed, and `cargo deny check bans` would fail on any other one (e.g. a hypothetical `lumen-model` dependency on `lumen-fixtures`). Before this fix, `deny.toml` never listed `lumen-store`/`lumen-fixtures` in `[bans].deny` at all, so `cargo deny check bans` passed regardless of this rule -- the check now actually enforces what this paragraph documents. `lumen-model` has no such dependency at all.
